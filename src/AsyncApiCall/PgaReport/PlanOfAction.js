...
export const savePlanOfAction = async (studentId, productId, data) => {
    try {
        const response = await customAxios.post(URL+"/api/v1/students/"+studentId+"/product/"+productId+"/pgaReport/poa", data)
        return response;
    } catch (error) {
        return error.response && error.response.data.message ? error.response.data.message : error.message
    }
...
export const deleteFocus = async (focusId) => {
    try {
        const response = await customAxios.delete(URL+"/api/v1/pga/poa/focus/"+focusId)
        return response;
    } catch (error) {
        return error.response && error.response.data.message ? error.response.data.message : error.message
    }
