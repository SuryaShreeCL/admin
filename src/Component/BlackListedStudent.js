...
import { getStudents, getStudentPaginate, postStudents, mernStudentSignUp, mernStudentEdit, getBlackListedUser } from "../Actions/Student";
...
export class BlackListedStudents extends Component {
  constructor(props) {
    super(props);
    this.state = {
      id : null,
  ...
  componentDidMount() {
    this.props.getBlackListedUser(0, 20);
  ...
  paginate = (page, size, keyword) => {
    ...
    if (
      ...
      this.props.getBlackListedUser(page, size, keyword);
    }
  };
  handleSubmit = (e) =>{
    ...
    return (
      <MuiThemeProvider theme={this.getmuitheme}>
        <div>
          ...
          {this.props.blackListedUserDetails.length !== 0 ? (
            <TableComponent
              data={
                this.props.blackListedUserDetails.length !== 0
                  ? this.props.blackListedUserDetails.content
                  : null
              }
              add={true}
              ...
              totalCount={this.props.blackListedUserDetails.totalElements}
              title={"Student"}
              pageCount={this.props.blackListedUserDetails.totalPages}
            />
          ) : (
...
const mapStateToProps = (state) => {
  return {
    ...
    blackListedUserDetails : state.StudentReducer.blackListedUserDetails,
...
export default connect(mapStateToProps, { getStudents, getStudentPaginate, postStudents, getAllColleges, getBranches, mernStudentSignUp, mernStudentEdit, getBlackListedUser })(
    BlackListedStudents
);
