                ...
                {props.studentProductList.length !== 0
                  ? props.studentProductList.map((singleProduct) => {
                      return (
                        <>
                          <TableRow>
                            <TableCell>{singleProduct.product.id}</TableCell>
                            <TableCell>{singleProduct.product.name}</TableCell>
                          </TableRow>
                        </>
                      );
                    })
            ...
            <Grid container spacing={2}>
              <Grid item md={12} sm={12} xs={12}>
                <Autocomplete
                  id="tags-outlined"
                  options={
                    props.productFamilyList !== undefined
                      ? props.productFamilyList
                      : []
                  }
                  getOptionLabel={(option) =>
                    option.productName === null ? "" : option.productName
                  }
                  filterSelectedOptions
                  onChange={(event, newValue) =>
                    productFamilyChangeHandler(newValue)
                  }
                  renderInput={(params) => (
                    <TextField
                      {...params}
              ...
              {props.productVariantList !== undefined &&
                    ...
                    <>
                      ...
                      <Grid item md={4} sm={4} xs={4}>
                        <Button
                          fullWidth
                          size="small"
                          onClick={() => submitHandler(eachProduct)}
                          variant="contained"
                          color={"primary"}
                        >
                          Allocate Product
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    addProductToStudentResponse:
      state.ProductReducer.addProductToStudentResponse,
  };
};
