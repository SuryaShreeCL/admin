...
import {
  getAllColleges, getBranches, getDegree, getUniversity
} from "../../Actions/College";
import {
  getAllEnrollmentPerid, getCareerInterest,
  getChoosenTrackById, getPackageByStudentId,
  getPbChoosenTrack, getPgaScores, postGenralDetails
...
class GeneralDetails extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      s3Link:
        "https://unifiedportalfiles-stage.s3.ap-south-1.amazonaws.com/" +
        this.props.id,
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.StudentDetails !== prevProps.StudentDetails) {
      this.setState({
        ...
        choosentrack:
          this.props.StudentDetails.chosenTrack !== null
            ? this.props.StudentDetails.chosenTrack
            : null,
        phoneNumber: this.props.StudentDetails.phoneNumber,
        enrollmentPeriod:
          this.props.StudentDetails.enrollmentPeriod !== null
            ? this.props.StudentDetails.enrollmentPeriod
            : null,
        expectedYear: {
        ...
        country:
          this.props.aspirationDetails.countries !== undefined
            ? this.props.aspirationDetails.countries
            : [],
      });
    }
    if (this.props.pgaScoreDetails !== prevProps.pgaScoreDetails) {
      this.setState({
        ...
        numericalAbility: this.props.pgaScoreDetails[
          "Numerical Ability (Score Out of 100)"
        ],
        logicalReasoning: this.props.pgaScoreDetails[
          "Logical Reasoning (Score Out of 100)"
        ],
        verbalReasoning: this.props.pgaScoreDetails[
          "Verbal Reasoning (Score Out of 100)"
        ],
        personalityCode: this.props.pgaScoreDetails.personalitycode
          .replace("[", "")
          .replace("]", ""),
      });
      let mechObj = this.props.pgaScoreDetails.score.find(
        (eachDetails) =>
          eachDetails.questionSetName === "Technical Test Mechanical"
      );
      ...
      let csObj = this.props.pgaScoreDetails.score.find(
        (eachDetails) =>
          eachDetails.questionSetName === "Technical Test Computer"
      );
      if (csObj !== undefined) {
        this.setState({
      ...
      let eceObj = this.props.pgaScoreDetails.score.find(
        (eachDetails) =>
          eachDetails.questionSetName === "Technical Test Electronics"
      );
      if (eceObj !== undefined) {
        this.setState({
    ...
    if (
      this.props.choosenTrackForStudent !== prevProps.choosenTrackForStudent
    ) {
    ...
    if (
      this.props.postGeneralDetailsResponse !==
      prevProps.postGeneralDetailsResponse
    ) {
      this.setState({
  ...
  handleSaved = () => {
    ...
    this.state.uggpa === null || this.state.uggpa.toString().length === 0
          ...
          <Grid item md={3}>
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  helperText={this.state.collegeHelperTxt}
                  label="College"
                  variant="outlined"
                />
              )}
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={4}>
                <Autocomplete
                  ...
                  onChange={(e, newValue) =>
                    this.setState({ choosentrack: newValue })
                  }
                  getOptionLabel={(option) => option.name}
                  size="small"
                  renderInput={(params) => (
              ...
              <Grid item md={4}>
                <Autocomplete
                  id="combo-box-demo"
                  options={this.props.enrollmentPeriod.filter(
                    (value) => value.name.lastIndexOf("1") === -1
                  )}
                  getOptionLabel={(option) => option.name}
                  size="small"
                  value={this.state.enrollmentPeriod}
                  onChange={(e, newValue) =>
                    this.setState({ enrollmentPeriod: newValue })
                  }
                  renderInput={(params) => (
                    <TextField
                      {...params}
