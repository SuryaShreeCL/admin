...
class Pgaplan extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.props.byTypeDetails !== prevProps.byTypeDetails) {
      this.props.byTypeDetails.map((eachData, index) => {
        this.setState({
          ["focus"
            .concat(eachData.focusNo + "-")
            .concat(eachData.enrollmentPeriod.name)]:
            eachData.course.name === "null" ? null : eachData.course.name,
    ...
    if (
      this.props.postPGaPlanCareerTrack !== prevProps.postPGaPlanCareerTrack
    ) {
      this.setState({
  ...
  handleChangeEdsin = (e, i, careerTrackIndex, specializationIndex, item) => {
    ...
    let removeExistCareerTrackArr = this.state.selectedSpecializationItem.filter(
      (item) => item.careerTrackIndex !== careerTrackIndex
    );
    let newCareerTrack = [];
    let existCareerTrack = this.state.selectedSpecializationItem.filter(
      (item) => item.careerTrackIndex === careerTrackIndex
    );
    let removeOldSpecialization = [];
    if (existCareerTrack.length > 0) {
      removeOldSpecialization = existCareerTrack[0].specialization.filter(
        (item) => item.specializationIndex !== specializationIndex
      );
    }
  ...
  handleCourseChange = (
    ...
    existCareerTrack = this.state.selectedSpecializationItem.filter(
      (item) => item.careerTrackIndex === careerTrackIndex
    );
    if (existCareerTrack.length > 0) {
      existSpecialization = existCareerTrack[0].specialization.filter(
        (item) => item.specializationIndex === specializationIndex
      );
      if (existSpecialization.length > 0) {
        existStarterPack = existSpecialization[0].starterPackCourse.filter(
          (item) => item.courseIndex !== courseIndex
        );
    ...
    let removeSpecialization = existCareerTrack[0].specialization.filter(
      (item) => item.specializationIndex !== specializationIndex
    );
    ...
    removeExistCareerTrackArr = removeExistCareerTrackArr.concat(
      existCareerTrackObj
    );
  ...
  obj = {
    description:
      "Consider doing an online course for additional skill building in the area of your interest. Check the links below for domain name-related courses. Consider completing at least one of the courses, getting a certification and working on a capstone project based on it.",
        ...
        focusSelectControl1: (
          <FormControl size="small" fullWidth variant="outlined">
            ...
            <Select
              ...
              value={
                // this.obj
                this.state["focus".concat(i + "-").concat(periodArr[0])] !==
                undefined
                  ? this.state["focus".concat(i + "-").concat(periodArr[0])]
                  : null
              }
        ...
        focusSelectControl2: (
          <FormControl size="small" fullWidth variant="outlined">
            ...
            <Select
              ...
              label="Select Option"
              value={
                this.state["focus".concat(i + "-").concat(periodArr[1])] !==
                undefined
                  ? this.state["focus".concat(i + "-").concat(periodArr[1])]
                  : null
              }
        ...
        focusSelectControl3: (
          <FormControl size="small" fullWidth variant="outlined">
            ...
            <Select
              ...
              label="Select Option"
              value={
                this.state["focus".concat(i + "-").concat(periodArr[2])] !==
                undefined
                  ? this.state["focus".concat(i + "-").concat(periodArr[2])]
                  : null
              }
        ...
        focusSelectControl4: (
          <FormControl size="small" fullWidth variant="outlined">
            ...
            <Select
              ...
              label="Select Option"
              value={
                this.state["focus".concat(i + "-").concat(periodArr[3])] !==
                undefined
                  ? this.state["focus".concat(i + "-").concat(periodArr[3])]
                  : null
              }
    ...
    return focusArr.map((eachElement) => {
      return (
        <>
          ...
          <Grid item md={2}>
            {eachElement.focusSelectControl1}
          </Grid>
          <Grid item md={2}>
            {eachElement.focusSelectControl2}
          </Grid>
          <Grid item md={2}>
            {eachElement.focusSelectControl3}
          </Grid>
          <Grid item md={2}>
            {eachElement.focusSelectControl4}
          </Grid>
          <Grid item md={2}></Grid>
        </>
      );
  ...
  handleSave = () => {
    ...
    this.props.postPgaPlanCareerTrack(
      this.props.id,
      this.state.selectedSpecializationItem
    );
                    ...
                    <IconButton
                      ...
                      onClick={() => {
                        ...
                        if (arr[0].row.length > 1) {
                          ...
                          //delete arr
                          let existCareerTrack = this.state.selectedSpecializationItem.filter(
                            (arr) => arr.careerTrackIndex === careerTrackIndex
                          );
                          let existSpecialization = [];
                          if (existCareerTrack[0] !== undefined) {
                            existSpecialization = existCareerTrack[0].specialization.sort(
                              (a, b) =>
                                a.specializationIndex > b.specializationIndex
                                  ? 1
                                  : b.specializationIndex >
                                    a.specializationIndex
                                  ? -1
                                  : 0
                            );
                          ...
                          let newCareerTrack = this.state.selectedSpecializationItem.filter(
                            (arr) => arr.careerTrackIndex !== careerTrackIndex
                          );
        ...
        <Grid container spacing={2} style={{ padding: "1%" }}>
          ...
          <Grid item md={12} style={{ padding: "1%" }}>
            <Autocomplete
              ...
              fullWidth
              onChange={(e, newValue) =>
                this.setState({ commonFocusFist: newValue })
              }
              size="small"
              renderInput={(params) => (
                <TextField
          ...
          <Grid item md={12} style={{ padding: "1%" }}>
            <Autocomplete
              ...
              fullWidth
              onChange={(e, newValue) =>
                this.setState({ commonFocusSecond: newValue })
              }
              size="small"
              renderInput={(params) => (
                <TextField
