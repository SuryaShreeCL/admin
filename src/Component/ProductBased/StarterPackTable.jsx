...
class StarterPackTable extends Component {
  ...
  handleEnroll = () => {
    ...
    isEmptyString(this.state.courseid)
      ? this.setState({ courseidErr: hlptxt })
      : this.setState({ courseidErr: "" });
    isEmptyString(this.state.enrollmentDate)
      ? this.setState({ enrollmentDateErr: hlptxt })
      : this.setState({ enrollmentDateErr: "" });
    isEmptyString(this.state.expiryDate)
      ? this.setState({ expiryDateErr: hlptxt })
      : this.setState({ expiryDateErr: "" });
    isEmptyString(this.state.track)
      ? this.setState({ trackErr: hlptxt })
      : this.setState({ trackErr: "" });
    isEmptyString(this.state.specialization)
      ? this.setState({ specializationErr: hlptxt })
      : this.setState({ specializationErr: "" });
    this.state.enrolledby !== undefined && isEmptyString(this.state.enrolledby)
      ? this.setState({ enrolledbyErr: hlptxt })
      : this.setState({ enrolledbyErr: "" });
    // isEmptyString(this.state.clsId) ? this.setState({ clsidErr : hlptxt}) : this.setState({ clsidErr : ""})
    this.state.expiryday !== undefined && isEmptyString(this.state.expiryday)
      ? this.setState({ expirydayErr: hlptxt })
      : this.setState({ expirydayErr: "" });
    this.state.expiryDate.getTime() <= this.state.enrollmentDate.getTime()
      ? this.setState({
          expiryDateErr: "Expiry date must be greater than enrollment date",
        })
      : this.setState({ expiryDateErr: "" });
    if (
          ...
          <Grid item md={12}>
            <TableContainer>
              <Table>
                ...
                {this.props.getenrollList.length !== 0 &&
                  this.props.getenrollList.map((eachData, index) => {
                    ...
                    return (
                      <TableRow>
                        ...
                        <TableCell align="center">{Enrolldate}</TableCell>
                        <TableCell align="center">
                          {eachData.SPCourse}
                        </TableCell>
                        <TableCell align="center">{eachData.status}</TableCell>
                        {/* <TableCell align="center">
                                        {eachData.cert}
                                    </TableCell> */}
                        <TableCell align="center">
                          <PrimaryButton
                            onClick={() => this.handleEdit(eachData)}
                            color={"primary"}
                            size={"small"}
                            variant={"contained"}
                          >
          ...
          >
            <DialogContent>
              <Grid container spacing={2}>
                ...
                <Grid item md>
                  <MuiPickersUtilsProvider utils={DateFnsUtils}>
                    <KeyboardDatePicker
                      ...
                      onChange={(value) =>
                        this.setState({ enrollmentDate: value })
                      }
                      KeyboardButtonProps={{
                ...
                <Grid item md>
                  <TextField label={"Days for Expiry"} />
                </Grid>
              </Grid>
              <Grid
                container
            ...
            <Grid container spacing={2}>
              ...
              <Grid item md={3} sm={6} lg={3}>
                <TextField
                  ...
                  onChange={(e) =>
                    this.setState({ enrolledby: e.target.value })
                  }
                  InputLabelProps={{
                    shrink: true,
                  }}
              ...
              <Grid item md={2} sm={6} lg={2}>
                <TextField
                  ...
                  onChange={(e, newValue) =>
                    this.setState({ expiryday: e.target.value })
                  }
                  InputLabelProps={{
                    shrink: true,
                  }}
