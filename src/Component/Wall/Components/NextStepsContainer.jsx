...
const NextStepsContainer = React.memo(
  ({ values, setFieldValue, setNotify }) => {
    ...
    const [statusFileUploadDisabled, setStatusFileUploadDisabled] = useState(
      false
    );
    ...
    const deleteWallStep = async (id) => {
      try {
        await axios.delete(
          `${process.env.REACT_APP_API_URL}/api/v1/wallsteps/${id}`,
          {
            crossDomain: true,
            headers: {
              admin: 'yes',
                    ...
                    >
                      ...
                      >
                        ...
                        <ErrorMessage
                          name={`wallSteps.${index}.heading`}
                          render={(msg) => (
                            <p className={classes.fieldErr}>{msg}</p>
                          )}
                        />
                      </div>
                      <div
